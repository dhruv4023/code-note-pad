...
export function NoteCard({ note, onEdit, onDelete }: NoteCardProps) {
  return (
    <div className="group rounded-xl border border-border bg-card p-4 space-y-3 animate-fade-in hover:border-primary/30 transition-colors">
      {/* Title & actions */}
      <div className="flex items-start justify-between gap-3">
        <h3 className="font-semibold text-sm leading-snug">{note.title}</h3>
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <Button variant="ghost" size="icon" className="h-7 w-7 rounded-md" onClick={() => onEdit(note)}>
            <Pencil className="w-3.5 h-3.5" />
          </Button>
          <Button variant="ghost" size="icon" className="h-7 w-7 rounded-md text-destructive" onClick={() => onDelete(note.id)}>
      ...
      {note.aiTags && note.aiTags.length > 0 && (
        <div className="flex items-center gap-1.5 flex-wrap pt-1">
          ...
          {note.aiTags.map((tag) => (
            <span key={tag} className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-tag-bg text-tag-foreground">
              {tag}
            </span>
          ))}
        </div>
